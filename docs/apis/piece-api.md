...
Both take at most two connections into any input they build. In installed Edge 153, two renders each of two 8-second soundtracks built from them, one of 15 voices and one of 138, each in a room, held the same bits. [The film scaffold](../../skills/artifex/film-scaffold.cjs) in the runtime skill wires them into a cue table. Run `node --test tests/sound.test.js` for the layers, their envelopes, the seeded noise and impulse, the refusals and the scaffold's cue table.
