...
An encoder's soundtrack does not start and end with the film. It opens with priming, samples the encoder needs to warm up, and its last packet runs past the film: a 2 s soundtrack encoded by installed Edge ends at 2.005 s as AAC and at 2.02 s as Opus. Each soundtrack therefore carries one edit (`edts/elst`). Its `media_time` skips the priming, and its `segment_duration` plays exactly the film's length, with media rate 1. The encoded samples themselves are all kept.
...
For AAC, an encoder reports its priming by stamping its first packet before zero, and the edit skips that many samples. Edge's AAC encoder stamps its first packet at zero, and its soundtrack decodes in Edge with no offset against the rendered sound, so its edit starts at 0 and only trims the end.

In installed Edge 153, both `decodeAudioData` and a `<video>` element play an exported test film's soundtrack from its first sample to the film's last, AAC and Opus alike; the element starts an AAC soundtrack 448 samples (9.3 ms) in, with or without an edit list. Firefox, Safari and FFmpeg-based players were not tested.
