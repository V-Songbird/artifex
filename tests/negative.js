...
// --- the accessors that were implemented and never once run ---------------
  ...
  },
  {
    ...
    file: 'core/film.js',
    from: 'Math.min(LOUDNESS.target - measured.lufs, ceiling - measured.dbtp)',
    to: 'LOUDNESS.target - measured.lufs',
    expect: 'every film soundtrack reaches -14 LUFS with one static gain, or stops at its codec ceiling, -1 dBTP for AAC and -1.2 for Opus',
  },
  {
    why: 'an Opus soundtrack stops at the AAC ceiling, and Opus lifts its peaks past -1 dBTP',
    file: 'core/film.js',
    from: 'ceiling: { mp4a: -1, Opus: -1.2 }',
    to: 'ceiling: { mp4a: -1, Opus: -1 }',
    expect: 'every film soundtrack reaches -14 LUFS with one static gain, or stops at its codec ceiling, -1 dBTP for AAC and -1.2 for Opus',
  },
  {
    why: 'the export levels every soundtrack for AAC, whatever codec encodes it',
    file: 'core/film.js',
    from: "normalizeLoudness(buffer, soundCodec.codec === 'opus' ? 'Opus' : 'mp4a')",
    to: "normalizeLoudness(buffer, 'mp4a')",
    expect: 'every film soundtrack reaches -14 LUFS with one static gain, or stops at its codec ceiling, -1 dBTP for AAC and -1.2 for Opus',
  },
  {
  ...
  },
  {
    ...
    expect: "a decoded soundtrack measures -14 LUFS or its codec's ceiling, Opus stays under -1 dBTP, and the report names a shortfall past 3 LU",
  },
  {
    ...
    file: 'tools/check-browser.js',
    from: '(Math.abs(heard.dbtp - ceiling) <= within && heard.lufs < -14)',
    to: '(Math.abs(heard.dbtp - ceiling) <= within)',
    expect: "a decoded soundtrack measures -14 LUFS or its codec's ceiling, Opus stays under -1 dBTP, and the report names a shortfall past 3 LU",
  },
  {
    why: 'the browser check holds an Opus soundtrack to the AAC ceiling, so settle\'s Opus film, stopped at -1.2 dBTP, fails',
    file: 'tools/check-browser.js',
    from: 'opus ? [-1.2, 0.15] : [-1, 0.1]',
    to: 'opus ? [-1, 0.15] : [-1, 0.1]',
    expect: "a decoded soundtrack measures -14 LUFS or its codec's ceiling, Opus stays under -1 dBTP, and the report names a shortfall past 3 LU",
  },
  {
    why: 'the browser check holds Opus within 0.1 dB of its ceiling, under the 0.14 dB Opus moved a peak in installed Edge',
    file: 'tools/check-browser.js',
    from: 'opus ? [-1.2, 0.15] : [-1, 0.1]',
    to: 'opus ? [-1.2, 0.1] : [-1, 0.1]',
    expect: "a decoded soundtrack measures -14 LUFS or its codec's ceiling, Opus stays under -1 dBTP, and the report names a shortfall past 3 LU",
  },
  {
    why: 'the browser check holds AAC only within 0.15 dB of its ceiling, wider than the 0.03 dB AAC moves a peak',
    file: 'tools/check-browser.js',
    from: 'opus ? [-1.2, 0.15] : [-1, 0.1]',
    to: 'opus ? [-1.2, 0.15] : [-1, 0.15]',
    expect: "a decoded soundtrack measures -14 LUFS or its codec's ceiling, Opus stays under -1 dBTP, and the report names a shortfall past 3 LU",
  },
  {
    why: 'the browser check passes an Opus soundtrack at -14 LUFS that decodes over -1 dBTP',
    file: 'tools/check-browser.js',
    from: '  if (opus && !(heard.dbtp < -1)) return',
    to: '  if (false) return',
    expect: "a decoded soundtrack measures -14 LUFS or its codec's ceiling, Opus stays under -1 dBTP, and the report names a shortfall past 3 LU",
  },
  {
    why: 'the browser check holds AAC under -1 dBTP too, so an AAC film its codec lifts a hair past its ceiling fails',
    file: 'tools/check-browser.js',
    from: 'if (opus && !(heard.dbtp < -1))',
    to: 'if (!(heard.dbtp < -1))',
    expect: "a decoded soundtrack measures -14 LUFS or its codec's ceiling, Opus stays under -1 dBTP, and the report names a shortfall past 3 LU",
  },
  {
  ...
  },
  {
    ...
    file: 'tools/replay.js',
    from: '  const gain = api.loudnessGain(api.loudness(rendered), codec);',
    to: '  const gain = -14 - api.loudness(rendered).lufs;',
    expect: 'replay levels a soundtrack with the gain the export applies',
  },
  {
    why: 'replay levels an Opus soundtrack for AAC, 0.2 dB louder than the export made it',
    file: 'tools/replay.js',
    from: '  const gain = api.loudnessGain(api.loudness(rendered), codec);',
    to: "  const gain = api.loudnessGain(api.loudness(rendered), 'mp4a');",
    expect: 'replay levels a soundtrack with the gain the export applies',
  },
  {
