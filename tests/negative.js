...
// --- the accessors that were implemented and never once run ---------------
  ...
  },
  {
    ...
    file: 'tools/check-browser.js',
    from: "    const film = await evaluateInPieces(client, '((frameMatch, soundMatch) => (' + inspectFilm.toString() + ')(' + JSON.stringify(name)",
    to: "    const film = await evaluate(client, '((frameMatch, soundMatch) => (' + inspectFilm.toString() + ')(' + JSON.stringify(name)",
