...
// --- the accessors that were implemented and never once run ---------------
  ...
  {
    why: 'a voice connects its layers straight to its panner, so three add up in an order that changes',
    file: 'core/sound.js',
    from: '  sumInto(ctx, layers, place);',
    to: '  for (const l of layers) l.connect(place);',
